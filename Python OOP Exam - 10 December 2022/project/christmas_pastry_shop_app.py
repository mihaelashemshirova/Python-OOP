...
class ChristmasPastryShopApp:
    ...
    def __init__(self):
        self.booths: List[Booth] = []   # [OpenBooth(), PrivateBooth()...]
        self.delicacies: List[Delicacy] = []   # [Gingerbread(), Stolen()...]
        self.income: float = 0
    ...
    def add_delicacy(self, type_delicacy: str, name: str, price: float):
        ...
        for type_of_delicacy, value in self.TYPE_OF_DELICACY.items():
            if type_of_delicacy == type_delicacy:
                new_delicacy = value(name, price)
    ...
    def add_booth(self, type_booth: str, booth_number: int, capacity: int):
        ...
        if type_booth not in self.TYPE_OF_BOOTH:
            raise Exception(f'{type_booth} is not a valid booth!')

        booth = self.TYPE_OF_BOOTH[type_booth](booth_number, capacity)
