...
class CarTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.car = Car('Audi', 'A7', 20, 80)

    def test_initializing(self):
        self.assertEqual('Audi', self.car.make)
        self.assertEqual('A7', self.car.model)
    ...
    def test_not_positive_fuel_consumption_error(self):
        with self.assertRaises(Exception) as ex:
            car = Car('Audi', 'A7', 0, 80)

        self.assertEqual('Fuel consumption cannot be zero or negative!', str(ex.exception))

    def test_not_positive_fuel_capacity_error(self):
        with self.assertRaises(Exception) as ex:
            car = Car('Audi', 'A7', 20, 0)

        self.assertEqual('Fuel capacity cannot be zero or negative!', str(ex.exception))

    def test_not_positive_fuel_amount_error(self):
        with self.assertRaises(Exception) as ex:
    ...
    def test_fuel_change_after_driving(self):
        self.car.refuel(1000)
        self.car.drive(10)
        self.assertEqual(78, self.car.fuel_amount)
